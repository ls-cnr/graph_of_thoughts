...
- Restituisce `cohesion_score` (float).
...
### `_generate_title(sentences: List[str])`
Genera un titolo rappresentativo per un gruppo di frasi, utilizzando un modello LLM configurato tramite `llm_config`.

```python
def _generate_title(self, sentences: List[str]) -> str:
    slt = SentenceListThought("titles_input")
    slt.values = {"sentences": sentences}
    gen = TitleGenerator(f"title_gen_{len(sentences)}", self.llm_config)
    gen.process({"input_sentences": slt})
    return gen.outputs[0].values["title"]
```

#### Caratteristiche  
- `sentences`: lista di frasi (stringhe).
- Restituisce un oggetto `TitleThought`.

#### Scopo  
Riassumere semanticamente un gruppo di pensieri in un titolo concettuale.
...
def build_reticle(self, thoughts: List[Thought]):
    for i, current in enumerate(thoughts):
        ...
        if best_coh > 0.5:
            ...
            self.node_counter +=1
            rel = "extension" if best_coh > 0.4 else "creation"
...
### `SentenceListThought`
Classe che rappresenta un input al generatore di titoli, contenente una lista di frasi.

```python
class SentenceListThought(Thought):
    @property
    ...
    if key == "sentences":
        return "\n".join(self._values["sentences"])
    raise KeyError
```

**Caratteristiche**:
- Estende: `Thought`
- Campo previsto: `sentences` (`List[str]`)
- Metodo chiave: `get_for_template()`

**Scopo**:
- Passare più frasi al generatore in formato strutturato per creare un titolo coerente.

---

### `TitleThought`
Classe che rappresenta il titolo generato a partire da un gruppo di frasi.

```python
class TitleThought(Thought):
    @property
    def schema(self) -> dict:
        return {
            "type": "object",
    ...
    def get_for_template(self, key: str) -> str:
        if key == "title":
            return str(self._values["title"])
        raise KeyError
```

**Caratteristiche**:
- Estende: `Thought`
- Campo previsto: `title` (`str`)
- Metodo chiave: `get_for_template()`

**Scopo**:
- Contenere il titolo generato dall'LLM, pronto per essere mostrato o archiviato.

---

### `TitleGenerator`
Classe che esegue la generazione automatica del titolo tramite LLM, a partire da un input di tipo `SentenceListThought`.

```python
class TitleGenerator(GoTGenerator):
    @property
    def mapping(self) -> Dict[str, Type[Thought]]:
        return {"input_sentences": SentenceListThought}
    ...
    def task_instruction(self) -> str:
        return """
        You are a title generation assistant. Your task is to read a group of related sentences and generate a concise and meaningful title (3–6 words) that captures their shared semantic core.

        Sentences: {input_sentences}

        Guidelines:
        1. Focus on the common idea or message expressed across the sentences.
        2. Avoid vague or overly broad titles—be as specific and informative as possible.
        3. Do NOT repeat the original sentences or use quotes.
        4. Use neutral, descriptive language—avoid sensationalism or emotional tone.
        5. Do NOT add information that is not clearly implied by the sentences.

        The title should serve as a thematic label that helps categorize or mentally index the group of sentences.
        Output only the final title, and nothing else.
        """
```

**Caratteristiche**:
- Estende: `GoTGenerator`
- Input: `SentenceListThought`
- Output: `TitleThought`
- Output Cardinality: Uno
- Prompt: istruzioni LLM dettagliate per sintesi e astrazione

**Scopo**:
- Generare un titolo originale che sintetizzi più frasi in un’idea unica.

---

# Reticolo Semantico con TAACO 

##  Componente interview_decontextualizer.py
...
def process_interview():
    ...
    # Step 4: Lattice Reticle (opzionale)
    import os
...
- [TAACO](https://github.com/nlsdfg/TAACO) installato correttamente
