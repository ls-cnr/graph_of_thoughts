...
class MyApp:
    def __init__(self, parent):
        ...
        self.spacer1 = tk.Label(parent, text="Tool for the Automatic Analysis of Cohesion", font=helv16,
                                background=color)
        self.spacer1.pack()
        ...
        self.sourcetextframe = tk.LabelFrame(self.checkboxframe, text="Source text analysis (optional)",
                                             background=color, width="45")
        self.sourcetextframe.pack(fill=tk.X, expand=tk.TRUE)

        self.summarylabel = tk.LabelFrame(self.sourcetextframe, height="1", width="45", padx="4",
                                          text="Your selected source text:", background=color)
        self.summarylabel.pack(fill=tk.X)

        summary_file_name = "(No Source Text Chosen)"

        self.summarylabelchosen = tk.Label(self.summarylabel, height="1", width="37", justify=tk.LEFT, padx="4",
                                           anchor=tk.W, font=helv14, text=summary_file_name)
        ...
        self.source_text_options = tk.LabelFrame(self.sourcetextframe, height="1", width="45", padx="4",
                                                 text="Source text options", background=color)
        self.source_text_options.pack(fill=tk.X)

        self.st1_var = tk.IntVar()
        self.st1 = tk.Checkbutton(self.source_text_options, text="Key item overlap", variable=self.st1_var,
                                  background=color)
        ...
        self.type_frame = tk.LabelFrame(self.checkboxframe, text="Lemma tokens to analyze for lexical overlap and TTR",
                                        background=color, width="45")
        self.type_frame.pack(expand=tk.TRUE)

        self.overlap_frame = tk.LabelFrame(self.checkboxframe, text="Lexical overlap options", background=color,
                                           width="45")
        self.overlap_frame.pack(fill=tk.X, expand=tk.TRUE)

        self.semantic_frame = tk.LabelFrame(self.checkboxframe, text="Semantic overlap options", background=color,
                                            width="45")
        ...
        self.diag_frame = tk.LabelFrame(self.myContainer1, text="Diagnostic output options", background=color,
                                        width="45")
        ...
        self.cb20 = tk.Checkbutton(self.semantic_frame, text="Synonym overlap", variable=self.cb20_var,
                                   background=color)
        ...
        self.cb22 = tk.Checkbutton(self.diag_frame, text="Output tagged files", variable=self.cb22_var,
                                   background=color)
        self.cb22.grid(row=1, column=2, sticky="W")
        self.cb22.select()

        self.cb23_var = tk.IntVar()
        self.cb23 = tk.Checkbutton(self.diag_frame, text="Output diagnostic file", variable=self.cb23_var,
                                   background=color)
        self.cb23.grid(row=1, column=1, sticky="W")
        self.cb23.select()

        self.source_text_list = ["null", self.st1_var, self.st2_var, self.st3_var,
                                 self.st4_var]  # the "null" here was just a filler so the ordered calls would line up

        self.var_list = ["null", self.cb1_var, self.cb2_var, self.cb3_var, self.cb4_var, self.cb5_var, self.cb6_var,
                         self.cb7_var, self.cb8_var, self.cb9_var, self.cb10_var, self.cb11_var, self.cb12_var,
                         self.cb13_var, self.cb14_var, self.cb15_var, self.cb16_var, self.cb17_var, self.cb18_var,
                         self.cb19_var, self.cb20_var, self.cb21_var, self.cb22_var, self.cb23_var]

        # the following will replace the lists:
        self.var_dict = {"sourceKeyOverlap": self.st1_var, "sourceLSA": self.st2_var, "sourceLDA": self.st3_var,
                         "sourceWord2vec": self.st4_var, "wordsAll": self.cb1_var, "wordsContent": self.cb2_var,
                         "wordsFunction": self.cb3_var, "wordsNoun": self.cb4_var, "wordsPronoun": self.cb5_var,
                         "wordsArgument": self.cb6_var, "wordsVerb": self.cb7_var, "wordsAdjective": self.cb8_var,
                         "wordsAdverb": self.cb9_var, "overlapSentence": self.cb10_var,
                         "overlapParagraph": self.cb11_var, "overlapAdjacent": self.cb12_var,
                         "overlapAdjacent2": self.cb13_var, "otherTTR": self.cb14_var,
                         "otherConnectives": self.cb15_var, "otherGivenness": self.cb16_var,
                         "overlapLSA": self.cb17_var, "overlapLDA": self.cb18_var, "overlapWord2vec": self.cb19_var,
                         "overlapSynonym": self.cb20_var, "overlapNgrams": self.cb21_var, "outputTagged": self.cb22_var,
                         "outputDiagnostic": self.cb23_var}

        self.box_list = [self.cb1, self.cb2, self.cb3, self.cb4, self.cb5, self.cb6, self.cb7, self.cb8, self.cb9,
                         self.cb10, self.cb11, self.cb12, self.cb13, self.cb14, self.cb15, self.cb16, self.cb17,
                         self.cb18, self.cb19, self.cb20, self.cb21]
        ...
        self.inputdirlabel = tk.LabelFrame(self.secondframe, height="1", width="45", padx="4",
                                           text="Your selected input folder:", background=color)
        self.inputdirlabel.pack()

        # Creates label that informs user which directory has been chosen
        directoryprompt = "(No Folder Chosen)"
        self.inputdirchosen = tk.Label(self.inputdirlabel, height="1", width="37", justify=tk.LEFT, padx="4",
                                       anchor=tk.W, font=helv14, text=directoryprompt)
        ...
        self.outputdirlabel = tk.LabelFrame(self.secondframe, height="1", width="45", padx="4",
                                            text="Your selected output filename:", background=color)
        ...
        self.outputdirchosen = tk.Label(self.outputdirlabel, height="1", width="37", justify=tk.LEFT, padx="4",
                                        anchor=tk.W, font=helv14, text=outdirectoryprompt)
        ...
        self.progress = tk.Label(self.progresslabelframe, height="1", width="45", justify=tk.LEFT, padx="4",
                                 anchor=tk.W, font=helv14, text=progress)
        self.progress.pack()
    ...
    def button2Click(self, event):
        ...
        self.outdirname = tkinter.filedialog.asksaveasfilename(parent=root, defaultextension=".csv",
                                                               initialfile="results", title='Choose Output Filename')
    ...
    def runprogram(self, event):
        self.poll(self.progress)
        start_thread(main, self.dirname, self.outdirname, self.var_dict, True,
                     self.summary_name)  # the True here is for the GUI
...
def main(indir, outdir, varDict, gui=False, source_text=False):
    ...
    overlap_box = checkBoxes(varDict,
                             ["wordsAll", "wordsContent", "wordsFunction", "wordsNoun", "wordsPronoun", "wordsArgument",
                              "wordsVerb", "wordsAdjective", "wordsAdverb"])
    ...
    if indir == "":
        ...
        def overlap_counter(header_list, index_list, name_suffix, list, seg_1,
                            seg_2):  # this completes all overlap functions:
            # print list
            ...
            if n_segments < 2:
                if seg_1 == True:
                    pre_header_list = ["adjacent_overlap_" + name_suffix,
                                       "adjacent_overlap_" + name_suffix + "_div_seg",
                                       "adjacent_overlap_binary_" + name_suffix]
                ...
                if seg_2 == True:
                    pre_header_list = ["adjacent_overlap_2_" + name_suffix,
                                       "adjacent_overlap_2_" + name_suffix + "_div_seg",
                                       "adjacent_overlap_binary_2_" + name_suffix]
                    for header in pre_header_list: header_list.append(header)
                    pre_index_list = [0, 0, 0]
                    for pre_index in pre_index_list: index_list.append(pre_index)
            ...
            for lines in word_list:
                ...
                for words in line[1:]:
                    ...
                    for sentences in target_list:  # iterates through sentences to ensure that sentence boundaries are not crossed
                        sentence = " " + " ".join(
                            sentences) + " "  # turns list of words into a string, adds a space to the beginning and end
                        # print sentence
            ...
            if len(text) < (window_length + 1):
                ...
                for x in range(len(text)):
                    ...
                    if len(small_text) < window_length:
                        break
                    denom += 1
                    sum_ttr += safe_divide(len(set(small_text)), float(
                        window_length))  # safe_divide(float(len(set(small_text))),float(len(small_text)))
        ...
        def keyness(target_list, frequency_list_dict, top_perc=None, out_dir="",
                    keyname=""):  # note that frequency_list_dict should be normed
            list = []
        ...
        def simple_proportion(target_text, ref_text, type, index_name=None, index_list=None,
                              header_list=None):  # each text is a list
        ...
        def lsa_similarity(text_list_1, text_list_2, lsa_matrix_dict, index_list=None, index_name=None,
                           header_list=None, lsa_type="fwd", nvectors=300):
        ...
        prelim_not_function = ["NN", "NNS", "NNP", "NNPS", "JJ", "JJR", "JJS", "RB", "RBR", "RBS", "VB", "VBD", "VBG",
                               "VBN", "VBP", "VBZ", "MD"]
        ...
        reason_and_purpose_ngram = ["that is why", "for this reason", "for that reason", "because of", "on account of",
                                    "so that"]
        ...
        if varDict["overlapWord2vec"] == True or varDict["sourceWord2vec"] == True:
            ...
            word2vec_dict = dicter_2(resource_path("COCA_newspaper_magazine_export_word2vec.csv"), "\t", " ",
                                     lower=True)
        ...
        for the_file in os.listdir(
                resource_path("para_files/")):  # this cleans out the old diagnostic file (if applicable)
        ...
        if source_text != "" and varDict["sourceKeyOverlap"] == True:
            ...
            mag_news_adj_quad_list = open(resource_path('mag_news_adj_list_quad_lemma_freq.csv'),
                                          errors="ignore").read()
            ...
            mag_news_v_n_quad_list = open(resource_path('mag_news_v_n_list_quad_lemma_freq.csv'),
                                          errors="ignore").read()
            ...
            mag_news_a_n_quad_list = open(resource_path('mag_news_a_n_list_quad_lemma_freq.csv'),
                                          errors="ignore").read()
            ...
            mag_news_keywords = keyness(source_pos_lem_dict["all"], mag_news_uni_F, top_perc=.1, out_dir=key_out_dir,
                                        keyname="mag_news_keywords.txt")
            mag_news_n_keywords = keyness(source_pos_lem_dict["noun"], mag_news_uni_F, top_perc=.1, out_dir=key_out_dir,
                                          keyname="mag_news_n_keywords.txt")
            mag_news_v_keywords = keyness(source_pos_lem_dict["verb"], mag_news_uni_F, top_perc=.1, out_dir=key_out_dir,
                                          keyname="mag_news_v_keywords.txt")
            mag_news_v_n_keywords = keyness(source_pos_lem_dict["verb_noun"], mag_news_uni_F, top_perc=.1,
                                            out_dir=key_out_dir, keyname="mag_news_v_n_keywords.txt")
            mag_news_adj_keywords = keyness(source_pos_lem_dict["adj"], mag_news_uni_F, top_perc=.1,
                                            out_dir=key_out_dir, keyname="mag_news_adj_keywords.txt")

            mag_news_bi_keywords = keyness(source_ngram_pos_lem_dict["bi_list"], mag_news_bi_F, top_perc=.1,
                                           out_dir=key_out_dir, keyname="mag_news_bi_keywords.txt")
            mag_news_tri_keywords = keyness(source_ngram_pos_lem_dict["tri_list"], mag_news_tri_F, top_perc=.1,
                                            out_dir=key_out_dir, keyname="mag_news_tri_keywords.txt")
            mag_news_quad_keywords = keyness(source_ngram_pos_lem_dict["quad_list"], mag_news_quad_F, top_perc=.1,
                                             out_dir=key_out_dir, keyname="mag_news_quad_keywords.txt")
            mag_news_n_bi_keywords = keyness(source_ngram_pos_lem_dict["n_list_bi"], mag_news_n_bi_F, top_perc=.1,
                                             out_dir=key_out_dir, keyname="mag_news_n_bi_keywords.txt")
            mag_news_adj_bi_keywords = keyness(source_ngram_pos_lem_dict["adj_list_bi"], mag_news_adj_bi_F, top_perc=.1,
                                               out_dir=key_out_dir, keyname="mag_news_adj_bi_keywords.txt")
            mag_news_v_bi_keywords = keyness(source_ngram_pos_lem_dict["v_list_bi"], mag_news_v_bi_F, top_perc=.1,
                                             out_dir=key_out_dir, keyname="mag_news_v_bi_keywords.txt")
            mag_news_v_n_bi_keywords = keyness(source_ngram_pos_lem_dict["v_n_list_bi"], mag_news_v_n_bi_F, top_perc=.1,
                                               out_dir=key_out_dir, keyname="mag_news_v_n_bi_keywords.txt")
            mag_news_a_n_bi_keywords = keyness(source_ngram_pos_lem_dict["a_n_list_bi"], mag_news_a_n_bi_F, top_perc=.1,
                                               out_dir=key_out_dir, keyname="mag_news_a_n_bi_keywords.txt")
            mag_news_n_tri_keywords = keyness(source_ngram_pos_lem_dict["n_list_tri"], mag_news_n_tri_F, top_perc=.1,
                                              out_dir=key_out_dir, keyname="mag_news_n_tri_keywords.txt")
            mag_news_adj_tri_keywords = keyness(source_ngram_pos_lem_dict["adj_list_tri"], mag_news_adj_tri_F,
                                                top_perc=.1, out_dir=key_out_dir,
                                                keyname="mag_news_adj_tri_keywords.txt")
            mag_news_v_tri_keywords = keyness(source_ngram_pos_lem_dict["v_list_tri"], mag_news_v_tri_F, top_perc=.1,
                                              out_dir=key_out_dir, keyname="mag_news_v_tri_keywords.txt")
            mag_news_v_n_tri_keywords = keyness(source_ngram_pos_lem_dict["v_n_list_tri"], mag_news_v_n_tri_F,
                                                top_perc=.1, out_dir=key_out_dir,
                                                keyname="mag_news_v_n_tri_keywords.txt")
            mag_news_a_n_tri_keywords = keyness(source_ngram_pos_lem_dict["a_n_list_tri"], mag_news_a_n_tri_F,
                                                top_perc=.1, out_dir=key_out_dir,
                                                keyname="mag_news_a_n_tri_keywords.txt")
            mag_news_n_quad_keywords = keyness(source_ngram_pos_lem_dict["n_list_quad"], mag_news_n_quad_F, top_perc=.1,
                                               out_dir=key_out_dir, keyname="mag_news_n_quad_keywords.txt")
            mag_news_adj_quad_keywords = keyness(source_ngram_pos_lem_dict["adj_list_quad"], mag_news_adj_quad_F,
                                                 top_perc=.1, out_dir=key_out_dir,
                                                 keyname="mag_news_adj_quad_keywords.txt")
            mag_news_v_quad_keywords = keyness(source_ngram_pos_lem_dict["v_list_quad"], mag_news_v_quad_F, top_perc=.1,
                                               out_dir=key_out_dir, keyname="mag_news_v_quad_keywords.txt")
            mag_news_v_n_quad_keywords = keyness(source_ngram_pos_lem_dict["v_n_list_quad"], mag_news_v_n_quad_F,
                                                 top_perc=.1, out_dir=key_out_dir,
                                                 keyname="mag_news_v_n_quad_keywords.txt")
            mag_news_a_n_quad_keywords = keyness(source_ngram_pos_lem_dict["a_n_list_quad"], mag_news_a_n_quad_F,
                                                 top_perc=.1, out_dir=key_out_dir,
                                                 keyname="mag_news_a_n_quad_keywords.txt")
        ...
        for filename in filenames:
                ...
                for sent in doc.sents:  # this iterates through sentences
                    ...
                    for token in sent:
                        ...
                        if POS_token in adverbs:
                            ...
                            if lemma_token in adj_word_list or (
                                    lemma_token[-2:] == "ly" and lemma_token[:-2] in adj_word_list):
                                diagnostic_token.append("CW")
                                content_text.append(lemma_token)
                                content_list_para.append(lemma_token)
                                content_list_sent.append(lemma_token)
            ...
            if varDict["otherTTR"] == True:
                ...
                if varDict["overlapNgrams"] == True:
                    ...
                    indexer(header_list, index_list, "bigram_lemma_ttr",
                            safe_divide(n_bigram_lemma_types, n_bigram_lemmas))
                    indexer(header_list, index_list, "trigram_lemma_ttr",
                            safe_divide(n_trigram_lemma_types, n_trigram_lemmas))
            ...
            if varDict["overlapSentence"] == True:
                if varDict["wordsAll"] == True:
                    overlap_counter(header_list, index_list, "all_sent", sent_lemma_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # all words, lemmatized
                if varDict["wordsContent"] == True:
                    overlap_counter(header_list, index_list, "cw_sent", sent_content_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # Content Words, Lemmatized
                if varDict["wordsFunction"] == True:
                    overlap_counter(header_list, index_list, "fw_sent", sent_function_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # Function Words, Lemmatized
                if varDict["wordsNoun"] == True:
                    overlap_counter(header_list, index_list, "noun_sent", sent_pos_noun_list,
                                    varDict["overlapAdjacent"], varDict["overlapAdjacent2"])  # POS NOUN, Lemmatized
                if varDict["wordsVerb"] == True:
                    overlap_counter(header_list, index_list, "verb_sent", sent_pos_verb_list,
                                    varDict["overlapAdjacent"], varDict["overlapAdjacent2"])  # POS Verb, Lemmatized
                if varDict["wordsAdjective"] == True:
                    overlap_counter(header_list, index_list, "adj_sent", sent_pos_adj_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # POS Adj, Lemmatized
                if varDict["wordsAdverb"] == True:
                    overlap_counter(header_list, index_list, "adv_sent", sent_pos_adv_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # POS Adv, Lemmatized
                if varDict["wordsPronoun"] == True:
                    overlap_counter(header_list, index_list, "pronoun_sent", sent_pos_prp_list,
                                    varDict["overlapAdjacent"], varDict["overlapAdjacent2"])  # POS PRP, Lemmatized
                if varDict["wordsArgument"] == True:
                    overlap_counter(header_list, index_list, "argument_sent", sent_pos_argument_list,
                                    varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # POS ARGUMENT (PRP + NOUN)

            ###BEGIN PARAGRAPH OVERLAP SECTION###
            if varDict["overlapParagraph"] == True:

                if varDict["wordsAll"] == True:
                    overlap_counter(header_list, index_list, "all_para", para_lemma_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # all words, lemmatized
                if varDict["wordsContent"] == True:
                    overlap_counter(header_list, index_list, "cw_para", para_content_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # Content Words, Lemmatized
                if varDict["wordsFunction"] == True:
                    overlap_counter(header_list, index_list, "fw_para", para_function_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # Function Words, Lemmatized
                if varDict["wordsNoun"] == True:
                    overlap_counter(header_list, index_list, "noun_para", para_pos_noun_list,
                                    varDict["overlapAdjacent"], varDict["overlapAdjacent2"])  # POS NOUN, Lemmatized
                if varDict["wordsVerb"] == True:
                    overlap_counter(header_list, index_list, "verb_para", para_pos_verb_list,
                                    varDict["overlapAdjacent"], varDict["overlapAdjacent2"])  # POS Verb, Lemmatized
                if varDict["wordsAdjective"] == True:
                    overlap_counter(header_list, index_list, "adj_para", para_pos_adj_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # POS Adj, Lemmatized
                if varDict["wordsAdverb"] == True:
                    overlap_counter(header_list, index_list, "adv_para", para_pos_adv_list, varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # POS Adv, Lemmatized
                if varDict["wordsPronoun"] == True:
                    overlap_counter(header_list, index_list, "pronoun_para", para_pos_prp_list,
                                    varDict["overlapAdjacent"], varDict["overlapAdjacent2"])  # POS PRP, Lemmatized
                if varDict["wordsArgument"] == True:
                    overlap_counter(header_list, index_list, "argument_para", para_pos_argument_list,
                                    varDict["overlapAdjacent"],
                                    varDict["overlapAdjacent2"])  # POS ARGUMENT (PRP + NOUN)
            ...
            if varDict["overlapSynonym"] == True:
                ...
                noun_sent_syn_lemma_overlap = syn_overlap(header_list, index_list, "sent_noun", sent_pos_noun_list,
                                                          noun_sent_syn_lemma_dict)
                verb_sent_syn_lemma_overlap = syn_overlap(header_list, index_list, "sent_verb", sent_pos_verb_list,
                                                          verb_sent_syn_lemma_dict)

                # Syn Paragraph Overlap Indices
                noun_para_syn_lemma_overlap = syn_overlap(header_list, index_list, "para_noun", para_pos_noun_list,
                                                          noun_para_syn_lemma_dict)
                verb_para_syn_lemma_overlap = syn_overlap(header_list, index_list, "para_verb", para_pos_verb_list,
                                                          verb_para_syn_lemma_dict)
            ...
            if varDict["otherConnectives"] == True:
                indexer(header_list, index_list, "basic_connectives",
                        safe_divide(ngram_counter(raw_text, basic_connectives), nwords))
                indexer(header_list, index_list, "conjunctions",
                        safe_divide(ngram_counter(raw_text, conjunctions), nwords))
                indexer(header_list, index_list, "disjunctions",
                        safe_divide(ngram_counter(raw_text, disjunctions), nwords))
                indexer(header_list, index_list, "lexical_subordinators", safe_divide(len(subordinators), nwords))
                indexer(header_list, index_list, "coordinating_conjuncts",
                        safe_divide(ngram_counter(raw_text, coordinating_conjuncts), nwords))
                ...
                indexer(header_list, index_list, "order",
                        safe_divide((len(order_list) + ngram_counter(raw_text, order_ngram)), nwords))
                indexer(header_list, index_list, "reason_and_purpose",
                        safe_divide((len(reason_and_purpose_list) + ngram_counter(raw_text, reason_and_purpose_ngram)),
                                    nwords))

                indexer(header_list, index_list, "all_causal", safe_divide(ngram_counter(raw_text, all_causal), nwords))
                indexer(header_list, index_list, "positive_causal",
                        safe_divide(ngram_counter(raw_text, positive_causal), nwords))

                indexer(header_list, index_list, "opposition", safe_divide(ngram_counter(raw_text, opposition), nwords))
                indexer(header_list, index_list, "determiners",
                        safe_divide(ngram_counter(raw_text, determiners), nwords))
                indexer(header_list, index_list, "all_demonstratives",
                        safe_divide(ngram_counter(raw_text, demonstratives), nwords))
                indexer(header_list, index_list, "attended_demonstratives",
                        safe_divide(len(attended_demonstratives_list), nwords))
                indexer(header_list, index_list, "unattended_demonstratives",
                        safe_divide(len(unattended_demonstratives_list), nwords))
                indexer(header_list, index_list, "all_additive",
                        safe_divide(ngram_counter(raw_text, all_additive), nwords))
                indexer(header_list, index_list, "all_logical",
                        safe_divide((len(all_logical_list) + ngram_counter(raw_text, all_logical_ngram)), nwords))
                indexer(header_list, index_list, "positive_logical",
                        safe_divide((len(positive_logical_list) + ngram_counter(raw_text, positive_logical_ngram)),
                                    nwords))
                indexer(header_list, index_list, "negative_logical",
                        safe_divide(ngram_counter(raw_text, negative_logical), nwords))
                indexer(header_list, index_list, "all_temporal",
                        safe_divide((len(all_temporal_list) + ngram_counter(raw_text, all_temporal_ngram)), nwords))
                indexer(header_list, index_list, "positive_intentional", safe_divide(
                    (len(positive_intentional_list) + ngram_counter(raw_text, positive_intentional_ngram)), nwords))
                indexer(header_list, index_list, "all_positive",
                        safe_divide((len(all_positive_list) + ngram_counter(raw_text, all_positive_ngram)), nwords))
                indexer(header_list, index_list, "all_negative",
                        safe_divide(ngram_counter(raw_text, all_negative), nwords))
                indexer(header_list, index_list, "all_connective",
                        safe_divide((len(all_connective_list) + ngram_counter(raw_text, all_connective_ngram)), nwords))

            ###END DICTIONARY LIST SECTION###

            ###Pronoun/Noun Ratio###
            if varDict["otherGivenness"] == True:
                indexer(header_list, index_list, "pronoun_density",
                        safe_divide((len(givenness_prp_text) + len(unattended_demonstratives_list)), nwords))
                indexer(header_list, index_list, "pronoun_noun_ratio",
                        safe_divide((len(givenness_prp_text) + len(unattended_demonstratives_list)), nnouns))

                ### GIVENNESS See DEFINED FUNCTIONS for givenness counters###
                indexer(header_list, index_list, "repeated_content_lemmas",
                        safe_divide(repeated_givenness_counter(content_text), nwords))
                indexer(header_list, index_list, "repeated_content_and_pronoun_lemmas", safe_divide((
                                                                                                                repeated_givenness_counter(
                                                                                                                    content_text) + repeated_givenness_counter(
                                                                                                            givenness_prp_text) + repeated_givenness_counter(
                                                                                                            unattended_demonstratives_list)),
                                                                                                    nwords))
            ...
            if source_text != "":
                if varDict["sourceKeyOverlap"] == True:  # key word overlap
                    ...
                    simple_proportion(lemma_text, mag_news_keywords, "perc", "mag_news_uni_keywords_percentage",
                                      index_list, header_list)
                    simple_proportion(noun_text, mag_news_n_keywords, "perc", "mag_news_n_uni_keywords_percentage",
                                      index_list, header_list)
                    simple_proportion(all_verb_text, mag_news_v_keywords, "perc", "mag_news_v_uni_keywords_percentage",
                                      index_list, header_list)
                    simple_proportion(all_verb_noun_text, mag_news_v_n_keywords, "perc",
                                      "mag_news_v_n_uni_keywords_percentage", index_list, header_list)
                    simple_proportion(adj_text, mag_news_adj_keywords, "perc", "mag_news_adj_uni_keywords_percentage",
                                      index_list, header_list)

                    simple_proportion(bi_list, mag_news_bi_keywords, "perc", "mag_news_bi_keywords_percentage",
                                      index_list, header_list)
                    simple_proportion(tri_list, mag_news_tri_keywords, "perc", "mag_news_tri_keywords_percentage",
                                      index_list, header_list)
                    simple_proportion(quad_list, mag_news_quad_keywords, "perc", "mag_news_quad_keywords_percentage",
                                      index_list, header_list)

                    simple_proportion(n_list_bi, mag_news_n_bi_keywords, "perc", "mag_news_n_bi_keywords_percentage",
                                      index_list, header_list)
                    simple_proportion(adj_list_bi, mag_news_adj_bi_keywords, "perc",
                                      "mag_news_adj_bi_keywords_percentage", index_list, header_list)
                    simple_proportion(v_list_bi, mag_news_v_bi_keywords, "perc", "mag_news_v_bi_keywords_percentage",
                                      index_list, header_list)
                    simple_proportion(v_n_list_bi, mag_news_v_n_bi_keywords, "perc",
                                      "mag_news_v_n_bi_keywords_percentage", index_list, header_list)
                    simple_proportion(a_n_list_bi, mag_news_a_n_bi_keywords, "perc",
                                      "mag_news_a_n_bi_keywords_percentage", index_list, header_list)

                    simple_proportion(n_list_tri, mag_news_n_tri_keywords, "perc", "mag_news_n_tri_keywords_percentage",
                                      index_list, header_list)
                    simple_proportion(adj_list_tri, mag_news_adj_tri_keywords, "perc",
                                      "mag_news_adj_tri_keywords_percentage", index_list, header_list)
                    simple_proportion(v_list_tri, mag_news_v_tri_keywords, "perc", "mag_news_v_tri_keywords_percentage",
                                      index_list, header_list)
                    simple_proportion(v_n_list_tri, mag_news_v_n_tri_keywords, "perc",
                                      "mag_news_v_n_tri_keywords_percentage", index_list, header_list)
                    simple_proportion(a_n_list_tri, mag_news_a_n_tri_keywords, "perc",
                                      "mag_news_a_n_tri_keywords_percentage", index_list, header_list)

                    simple_proportion(n_list_quad, mag_news_n_quad_keywords, "perc",
                                      "mag_news_n_quad_keywords_percentage", index_list, header_list)
                    simple_proportion(adj_list_quad, mag_news_adj_quad_keywords, "perc",
                                      "mag_news_adj_quad_keywords_percentage", index_list, header_list)
                    simple_proportion(v_list_quad, mag_news_v_quad_keywords, "perc",
                                      "mag_news_v_quad_keywords_percentage", index_list, header_list)
                    simple_proportion(v_n_list_quad, mag_news_v_n_quad_keywords, "perc",
                                      "mag_news_v_n_quad_keywords_percentage", index_list, header_list)
                    simple_proportion(a_n_list_quad, mag_news_a_n_quad_keywords, "perc",
                                      "mag_news_a_n_quad_keywords_percentage", index_list, header_list)

                if varDict["sourceLSA"] == True:  # lsa
                    indexer(header_list, index_list, "source_similarity_lsa",
                            lsa_similarity(lemma_text, source_pos_lem_dict["all"], lsa_dict))

                if varDict["sourceLDA"] == True:  # lda
                    indexer(header_list, index_list, "source_similarity_lda",
                            lda_divergence(lemma_text, source_pos_lem_dict["all"], lda_dict))

                if varDict["sourceWord2vec"] == True:  # word2vec
                    indexer(header_list, index_list, "source_similarity_word2vec",
                            lsa_similarity(lemma_text, source_pos_lem_dict["all"], word2vec_dict))
        ...
        if system == "M" and gui == True:
            tkinter.messagebox.showinfo("Finished!",
                                        "TAACO has converted your files to numbers.\n\n Now the real work begins!")
